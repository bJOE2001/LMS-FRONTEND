<template>
  ...
  >
    ...
    <q-card flat bordered :class="['rounded-borders q-mb-lg', { 'dialog-form-card': inDialog }]">
      <!-- Form Header -->
      <q-card-section v-if="!inDialog" class="text-center bg-green-8 text-white">
...
function applicationMatchesSelectedEmployee(application) {
  ...
  const selectedFirstName = normalizeEmployeeName(form.value.firstName)
  const selectedLastName = normalizeEmployeeName(form.value.lastName)
  const applicationEmployeeName = normalizeEmployeeName(getApplicationEmployeeName(application))

  if (!applicationEmployeeName || !selectedFirstName || !selectedLastName) {
    return false
  }

  return applicationEmployeeName.includes(selectedFirstName) && applicationEmployeeName.includes(selectedLastName)
}
...
function getApplicationMergeKey(application, index) {
  ...
  const employeeKey = getApplicationEmployeeLookupValue(application)
  const employeeName = normalizeEmployeeName(getApplicationEmployeeName(application))
  ...
  const statusKey = String(application?.rawStatus || application?.status || '')
    .trim()
    .toUpperCase()

  const fallbackKey = [employeeKey || employeeName, startDate, endDate, selectedDatesKey, statusKey]
    .filter(Boolean)
    .join('|')
...
function getApplicationLogicalKey(application, index) {
  ...
  return fallbackKey ? `logical:${fallbackKey}` : getApplicationMergeKey(application, index)
}
...
async function refreshDepartmentApplications() {
  ...
  departmentApplications.value = Array.from(mergedApplications.values())
}

function handleDepartmentApplicationsFocusRefresh() {
  refreshDepartmentApplications().catch(() => {})
}

function handleDepartmentApplicationsVisibilityRefresh() {
...
const employeeInformationalApplications = computed(() =>
  employeeApplicationsForBalance.value.filter((application) => getInformationalLeaveApplicationState(application)),
)
...
const lockedLeaveDateStates = computed(() => {
  ...
  employeeExistingApplications.value.forEach((application) => {
...
const informationalLeaveDateStates = computed(() => {
  ...
  employeeInformationalApplications.value.forEach((application) => {
...
function normalizeEmploymentStatusKey(status) {
...
function getAllowedLeaveTypesForEmployee(controlNo = selectedEmployeeControlNo.value || form.value.employeeControlNo) {
  ...
  const statusKey = normalizeEmploymentStatusKey(
    employee?.employment_status_key ?? employee?.raw_status ?? employee?.status,
  )

  return allLeaveTypes.value.filter((leaveType) => {
    const allowedStatuses = Array.isArray(leaveType?.allowed_status) ? leaveType.allowed_status : []
    if (!allowedStatuses.length) return true
    if (!statusKey) return false
    return allowedStatuses.includes(statusKey)
  })
}

function getDefaultLeaveTypeId(leaveTypes = getAllowedLeaveTypesForEmployee()) {
  const vacationType = leaveTypes.find((leaveType) => leaveType.name === 'Vacation Leave')
  if (vacationType?.id) return vacationType.id
...
function refreshLeaveTypeOptions() {
  ...
  if (selectedLeaveTypeStillAllowed) {
    return
  }
...
onMounted(async () => {
  ...
  try {
    ...
    await refreshDepartmentApplications()
    ...
    if (typeof window !== 'undefined') {
      window.addEventListener('focus', handleDepartmentApplicationsFocusRefresh)
    ...
    if (typeof document !== 'undefined') {
      document.addEventListener('visibilitychange', handleDepartmentApplicationsVisibilityRefresh)
    }
  } catch (err) {
...
watch(
  ...
  (employee) => {
    if (!employee?.control_no) return
    applyPrefilledEmployee(employee)
  },
)
...
function getLeaveBalanceWarningForTotal(totalDays) {
  ...
  if (isCtoType.value) {
    if (pendingSelectedLeaveDays.value > 0) {
      return `This employee already has ${formatDayCountValue(pendingSelectedLeaveDays.value)} day(s) of ${selectedLeaveTypeName.value} pending. CTO stays WP-only, so reduce the selected dates or wait for more valid CTO credits.`
    }
  ...
  if (pendingSelectedLeaveDays.value > 0) {
    return `This employee already has ${formatDayCountValue(pendingSelectedLeaveDays.value)} day(s) of ${selectedLeaveTypeName.value} pending. Automatic split for this request: ${formatDayCountValue(withPayDays)} day(s) WP and ${formatDayCountValue(withoutPayDays)} day(s) WOP.`
  }

  return `This employee's ${selectedLeaveTypeName.value} balance is insufficient. Automatic split for this request: ${formatDayCountValue(withPayDays)} day(s) WP and ${formatDayCountValue(withoutPayDays)} day(s) WOP.`
}
...
function onLeaveTypeChange(newValue) {
  ...
  form.value.vacationDetail = ['Vacation Leave', 'Special Privilege Leave'].includes(selectedLeaveType?.name || '')
...
function isWeekendDate(date) {
  const parsedDate = parseIsoDateValue(date)
...
watch(unavailableLeaveDates, (dates) => {
  const invalidSelectedDate = selectedDatesList.value.find((date) => dates.has(date))
  if (invalidSelectedDate) {
    selectedDates.value = selectedDatesList.value.filter((date) => dates.has(date) === false)
  }
...
onBeforeUnmount(() => {
  ...
  if (typeof window !== 'undefined') {
    window.removeEventListener('focus', handleDepartmentApplicationsFocusRefresh)
  }
  if (typeof document !== 'undefined') {
    document.removeEventListener('visibilitychange', handleDepartmentApplicationsVisibilityRefresh)
  }
})
...
watch(selectedDateTotalDays, (total) => {
  if (selectedDatesList.value.length === 0) return
