  ...
  <q-page class="q-pa-md">
    ...
    <q-dialog v-model="showExportDialog">
      <q-card style="width: 400px; max-width: 90vw;">
        <q-card-section>
          <div class="text-h6">Export Released Applications</div>
        </q-card-section>
...
async function performExport() {
  ...
  try {
    const { data } = await api.get('/hr/releasing/export', {
      params: {
    ...
    if (exportFormat.value === 'excel') {
      exportReleasingExcel(applications, dateRangeLabel)
    } else {
      await exportReleasingPdf(applications, dateRangeLabel)
    }
