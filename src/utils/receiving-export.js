...
export function exportReceivingExcel(applications, dateRangeLabel = 'Date Range') {
  const columns = [
    { name: 'no', label: 'No.' },
    ...
    { name: 'received_at', label: 'Date Received', field: (row) => {
        if (!row.received_at) return ''
        return new Date(row.received_at).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' })
    } },
  ]
  ...
  exportReportsMonitoringExcel({
    columns,
    rows: applications,
    reportLabel: `Receiving Applications ${dateRangeLabel}`
  })
}

export async function exportReceivingPdf(applications, dateRangeLabel = 'Date Range') {
  const columns = [
    ...
    { name: 'received_at', label: 'Date Received', width: 'auto' },
  ]
  ...
  const body = applications.map((row, index) => {
    return [
      ...
      { 
        text: row.received_at ? new Date(row.received_at).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }) : '',
        alignment: 'center'
      },
    ]
  })
  ...
  const docDefinition = {
    ...
    content: [
      ...
      {
        text: 'RECEIVING APPLICATIONS REPORT',
        style: 'subheader',
        alignment: 'center',
        margin: [0, 15, 0, 5]
      },
  ...
  const fileName = buildExportFilename(`Receiving Applications ${dateRangeLabel}`, 'pdf')
